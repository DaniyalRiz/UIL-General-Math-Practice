    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-0 mb-8 border-b border-slate-200 dark:border-slate-800">
        <button onClick={() => setSubtab('overview')}
          className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${subtab === 'overview' ? 'border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          Overview
        </button>
        <button onClick={() => setSubtab('achievements')}
          className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${subtab === 'achievements' ? 'border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          Achievements
          <span className="ml-1.5 text-xs font-bold px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">{earnedAch.length}/{achievementsData.length}</span>
        </button>
      </div>
      ...
      {subtab === 'overview' ? (
        <div className="space-y-6">
          ...
          {/* Topic breakdown */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
            <h2 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-5">Progress by Topic</h2>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
            <h2 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-5">Mastery Levels</h2>
        ...
        <div className="space-y-8">
          {earnedAch.length > 0 && (
            <div>
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-3">Earned · {earnedAch.length}</h2>
          ...
          {inProgressAch.length > 0 && (
            <div>
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-3">In Progress · {inProgressAch.length}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {lockedAch.length > 0 && (
            <div>
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-3">Locked · {lockedAch.length}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
